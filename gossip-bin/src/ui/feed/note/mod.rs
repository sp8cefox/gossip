                    ...
                    let footer_response = Frame::none()
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.horizontal(|ui| {
                                ...
                                render_cats(app,ui, note.event.id.as_hex_string());
...
fn render_cats(app: &mut GossipUi ,ui: &mut Ui, note_id: String) {
    ...
    let mut count = 0;
    let cat_iter = app.cat_list
        .clone()
        .into_iter();

    cat_iter.for_each(|cat|{
        ui.horizontal(|ui|{
            if !cat.has_conditions() || check_conditions(app, ui, cat.clone(), note_id.clone()){
                if widgets::clickable_label(
                    ui,
                    true,
                    RichText::new(cat.icon_uri.clone()).size(13.0),
                )
                .on_hover_text(cat.name.clone())
                .clicked()
                {
                    <std::option::Option<CatServiceFS> as Clone>::clone(&app.cat_service).unwrap()
                        .create_cat_element(cat.clone(), note_id.clone());
                    load_cat_elements(app);
                }
            } 
        });
    });
...
fn check_conditions(app: &mut GossipUi, ui: &mut Ui, cat: srf_cat::cat::Cat, note_id: String ) -> bool {
    
    let mut result = false;
    cat.conditioned_list
        .iter()
        .for_each(|condition_id| {
            if check_condition(app, ui, condition_id.to_string(), cat.point.clone(), note_id.clone()) {
                result = true;
            }
        });
    result
...
fn load_cat_elements(app: &mut GossipUi) {
    app.cat_list
        .clone()
        .into_iter()
        .for_each(|c| {
...
/// Checks if a given cat_id has been tagged with a condition_id
fn check_condition(app: &mut GossipUi, ui: &mut Ui, condition_id: String, cat_id: String, note_id: String) -> bool {
    
    app.loaded_cat_elements
        .clone()
        .get(&condition_id)
        .unwrap_or(&CatElementList::new())
        .get_point_ids()
        .contains(&note_id)
}
...
fn check_cat_service(app: &mut GossipUi) {
    ...
    //Seems some how the paint iteration for this one is one behind, so we call load twice. It's ugly, but it
    //does the job for now.
    if app.has_cat_elements_loaded < 2 {
        println!("gonna call load_cat_elements now... btw has_cat_elements_loaded = {}", app.has_cat_elements_loaded);
        load_cat_elements(app);
        app.has_cat_elements_loaded += 1;
    }
