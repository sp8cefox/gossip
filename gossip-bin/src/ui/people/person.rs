...
fn content(app: &mut GossipUi, ctx: &Context, ui: &mut Ui, pubkey: PublicKey, person: Person) {
    ...
    ui.with_layout(egui::Layout::left_to_right(egui::Align::TOP), |ui| {
        ...
        ui.with_layout(egui::Layout::top_down(egui::Align::TOP), |ui| {
          ...
          // This part could supposedly set width but something's missing for it to work.      
          ...
          //  ui.set_max_width(width);
              ...
              render_buttons(ui, app, person.clone());
            });
...
fn render_buttons(ui: &mut Ui, app: &mut GossipUi, person: Person) {

    let person_key = &person.pubkey.as_hex_string().clone();
    let mut user_key = String::from("");
    if let Some(pubkey) = GLOBALS.identity.public_key() {
        user_key = pubkey.as_hex_string();
    }

    //TODO:: In the future collect all magic values in an enum or something.
    let table_name: String = String::from("main_table");
    app.cat_list_map.insert(
        person_key.to_string(),     
        CatSingleServiceFS::get_all_cats_w_conditions(
            person_key.clone(),
            table_name.clone()
        )
    );

    app.cat_list_map.get(person_key)
    .unwrap_or(&CatList::new())
    .clone()
    .into_iter()
    .for_each(|cat| {
        ui.add_space(4.0);
        ...
        .clicked()
        {
            if *person_key != user_key {
                CatSingleServiceFS::create_cat(user_key.clone(), &cat.point, &cat.name, &cat.icon_uri, &cat.description);
                app.has_cat_combo_loaded = false;
            } else {
                // We don't fork our own buttons 
            }
        }
    });
}

fn check_cat_service(app: &mut GossipUi, pubkey: PublicKey) {
